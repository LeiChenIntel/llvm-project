...
static Value insertAllocAndDealloc(MemRefType type, Location loc,
  ...
  auto alloc = rewriter.create<AllocOp>(
      loc, type); // Need "mlir/Dialect/MemRef/IR/MemRef.h"
  ...
  auto dealloc = rewriter.create<DeallocOp>(
      loc, alloc); // memref::DeallocOp is in "mlir/Dialect/MemRef/IR/MemRef.h"
...
struct StandaloneToAffineLoweringPass
  ...
  void getDependentDialects(DialectRegistry &registry) const override {
    registry
        .insert<AffineDialect, StandardOpsDialect>(); // TAG: Register dialect
                                                      // and operation here
  }
    ...
    std::function<void(uint64_t)> storeElements = [&](uint64_t dimension) {
      ...
      if (dimension == valueShape.size()) {
        rewriter.create<AffineStoreOp>(
            loc, rewriter.create<ConstantOp>(loc, *valueIt++),
            alloc, // The ConstantOp is a mlir ConstantOp
...
struct ReturnOpLowering : public OpRewritePattern<standalone::ReturnOp> {
  ...
  LogicalResult matchAndRewrite(standalone::ReturnOp op,
    ...
    rewriter.replaceOpWithNewOp<ReturnOp>(
        op); // The ReturnOp is a mlir ReturnOp
    return success();
  }
};
...
struct MatmulOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    SmallVector<int64_t, 4> lowerBounds(numNests, 0); // (0, 0, 0)
    SmallVector<int64_t, 4> upperBounds{lhsShape[0], rhsShape[1],
                                        rhsShape[0]}; // (2, 2, 3)
    SmallVector<int64_t, 4> steps(numNests, 1); // (1, 1, 1)

    buildAffineLoopNest(
        rewriter, loc, lowerBounds, upperBounds, steps,
        [&](OpBuilder &nestedBuilder, Location loc, ValueRange ivs) {
          standalone::MatmulOp::Adaptor matmulAdaptor(operands);
